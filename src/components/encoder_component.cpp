...
Encoder::Encoder(std::shared_ptr<VMXPi> vmx, const std::string &name, VMXChannelIndex port_a, VMXChannelIndex port_b, const std::string &topic) 
    ...
    publisher_ = this->create_publisher<studica_ros2_control::msg::EncoderMsg>(topic, 10);
...
void Encoder::cmd_callback(std::shared_ptr<studica_ros2_control::srv::SetData::Request> request, std::shared_ptr<studica_ros2_control::srv::SetData::Response> response) {
    std::string params = request->params;
    cmd(params, response);
}

void Encoder::cmd(std::string params, std::shared_ptr<studica_ros2_control::srv::SetData::Response> response) {
    if (params == "get_count") {
...
void Encoder::publish_data() {
    auto msg = studica_ros2_control::msg::EncoderMsg();
...
RCLCPP_COMPONENTS_REGISTER_NODE(studica_ros2_control::Encoder)
